...
function getUsersFromDatabase(array) {
	  const usersArray = Array.from(array, (user) => {
		const { id, name, age } = user;
		return { id, firstName: name.toUpperCase(), years: age };
	  });
...
function copyAndSwapElements(arr, target, start, end) {
	if (target < 0 || start < 0 || end < 0 || target >= arr.length || start >= arr.length || end > arr.length) {
...
function customFill(arr, value, start = 0, end = arr.length) {
	if (!Array.isArray(arr) || start < 0 || end < 0 || start > arr.length || end > arr.length) {
		return false;
	  }
